...
class ArrayExp(object):
    ...
    def __init__(self, name, inWidth, outWidth, addr, size, func, content):
...
class Node(object):
    ...
    gfBase = 3
    gfIrr = 0x1b
    powersTwo = [1, 2, 4, 8, 16, 32, 64, 128]
    ...
    def setVarsOccurrences(self):
        if isinstance(self, ConstNode):
            assert(False) # to remove this 'if'
            return
        if isinstance(self, SymbNode):
            ...
            elif self.symbType == 'A':
                self.shareOcc[self.origSecret] = {}
                self.shareOcc[self.origSecret][self] = 1
            return
        for child in self.children:
            ...
            for k in child.secretVarOcc:
                if k in self.secretVarOcc:
                    ...
                    self.secretVarOcc[k] = child.secretVarOcc[k]
            for s in child.shareOcc:
                if s not in self.shareOcc:
                    self.shareOcc[s] = {}
                for a in child.shareOcc[s]:
                    if a in self.shareOcc[s]:
                        self.shareOcc[s][a] += child.shareOcc[s][a]
                    else:
                        self.shareOcc[s][a] = child.shareOcc[s][a]
    ...
    def __and__(self, other):
        e = Node.makeBitwiseNode('&', [self, other])
        return e
    ...
    def __invert__(self):
        n = Node.makeBitwiseNode('~', [self])
        return n

    def __add__(self, other):
        n = Node.makeBitwiseNode('+', [self, other])
        return n

    @staticmethod
    def makeBitwiseNode(op, children):
...
class OpNode(Node):
    def __init__(self, op, children):
        assert(children != None and children)
        self.op = op
        self.hasWordOp = (op == '*' or op == '**' or op == 'M' or op == 'P' or op == 'L' or op == 'X')
        ...
        if op == '+' or op == '--' or op == '-' or op == '&' or op == '|' or op == '^' or op == '~' or op == '<<' or op == '>>' or op == 'LS' or op == '*' or op == '**' or op == 'M' or op == 'P' or op == 'L' or op == 'X':
    ...
    def expPrint(self, parNeeded, verbatim):
        ...
        if self.op == '~':
            ...
            res = 'GMul(' + ', '.join(map(lambda x:x.expPrint(False, verbatim), self.children)) + ')'
            return res
        elif self.op == 'P':
            res = 'GPow(' + self.children[0].expPrint(False, verbatim) + ', ' + self.children[1].expPrint(False, verbatim) + ')'
            return res
        elif self.op == 'L':
            res = 'GLog(' + self.children[0].expPrint(False, verbatim) + ')'
            return res
        elif self.op == 'X':
            res = 'GExp(' + self.children[0].expPrint(False, verbatim) + ')'
            return res
...
def ZeroExt(numZeros, child):
    ...
    n = OpNode('C', [child, Const(0, numZeros)])
    return n
...
def SignExt(numSignBits, child):
    ...
    childrenList = []
    e = Extract(child.width - 1, child.width - 1, child)
    for i in range(numSignBits):
        childrenList.append(e)
    childrenList.append(child)
    n = OpNode('C', childrenList[::-1])
    return n
...
def GPow(c0, c1):
    assert(isinstance(c0, Node))
    assert(isinstance(c1, Node))
    if propagateCstOnBuild():
        if isinstance(c0, ConstNode) and isinstance(c1, ConstNode):
            return gpow(c0, c1)
    n = OpNode('P', [c0, c1])
    return n
