...
''' % (nbShares, currentScript, article, nbShares)
    ...
    for i in range(nbShares):
        for j in range(i + 1, nbShares):
            ...
            content += '    r%d_%d_p = inputGate(n_r%d_%d_p)\n' % (i, j, i, j)
    content += '    cst1 = inputGate(constant(1, 1))\n'
    content += '\n'
    ...
    for i in range(nbShares):
        for j in range(nbShares):
            if(j != i):
                content += '    u%d_%d = Register(xorGate(%s%d, r%d_%d))\n' % (i, j, inputVars[1], j, i, j)
    content += '\n'

    for i in range(nbShares):
        for j in range(nbShares):
            if(j != i):
                content += '    v%d_%d = Register(xorGate(andGate(xorGate(%s%d, cst1), r%d_%d), r%d_%d_p))\n' % (i, j, inputVars[0], i, i, j, i, j)
    content += '\n'

    for i in range(nbShares):
        for j in range(nbShares):
            if(j != i):
                content += '    w%d_%d = xorGate(andGate(Register(%s%d), u%d_%d), v%d_%d)\n' % (i, j, inputVars[0], i, i, j, i, j)
    content += '\n'
        ...
        for j in range(nbShares):
            if(j != i):
                if(j == 0 or (i == 0 and j == 1)):
                    content += '    sum%d_%d = w%d_%d\n' % (i, j, i, j)
                elif(j == i+1):
                    content += '    sum%d_%d = xorGate(sum%d_%d, w%d_%d)\n' % (i, j, i, j - 2, i, j)
                else:
                    content += '    sum%d_%d = xorGate(sum%d_%d, w%d_%d)\n' % (i, j, i, j - 1, i, j)
                lastj = j 
