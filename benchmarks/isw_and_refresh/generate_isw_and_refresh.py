...
outfile = 'isw_and_refresh_gen.py'


def usage():
    print('Usage: %s [options]' % os.path.basename(__file__))
    print('   This script generates a LeakageVerif file describing a circuit implementing the logical AND following the ISW scheme with share refreshing.')
    print('Options:')
...
'''
    ...
    for i in range(nbShares):
        content += 'hh%d = xorGate(r%d, r%d)\n' % (i, i, (i + 1) % nbShares)
    content += '\n'
    
    for i in range(nbShares):
        content += 'h%d = xorGate(hh%d, %s%d)\n' % (i, i, inputVars[0], i)
    content += '\n'
